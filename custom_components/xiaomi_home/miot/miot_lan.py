...
import struct
import threading
from typing import Any, Callable, Coroutine, Dict, List, Optional, final
...
class MIoTLan:
    ...
    async def update_net_ifs_async(self, net_ifs: list[str]) -> None:
        ...
        if not self._init_done:
            ...
            return
        self._internal_loop.call_soon_threadsafe(self.__update_net_ifs, net_ifs)
    ...
    def update_devices(self, devices: dict[str, dict]) -> bool:
        ...
        if not self._init_done:
            return False
        self._internal_loop.call_soon_threadsafe(self.__update_devices, devices)
        return True
        ...
        for did, info in devices.items():
            ...
            if did not in self._lan_devices:
                if "token" not in info:
                    _LOGGER.error("token not found, %s, %s", did, info)
                    continue
