...
from .const import (UNSUPPORTED_MODELS, DEFAULT_OAUTH2_API_HOST,
                    MIHOME_HTTP_API_TIMEOUT, OAUTH2_AUTH_URL)
    ...
    async def __get_device_list_page_async(
        ...
        for device in res_obj.get("list", []) or []:
            ...
            if model in UNSUPPORTED_MODELS:
                _LOGGER.info('ignore unsupported model %s, cloud, %s', model,
                             did)
                continue
            device_infos[did] = {
                "did":
