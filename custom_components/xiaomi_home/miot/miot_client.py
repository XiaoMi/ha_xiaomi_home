...
class MIoTClient:
    ...
    def __init__(
        ...
        self._ctrl_mode = CtrlMode.load(entry_data.get("ctrl_mode", DEFAULT_CTRL_MODE))
    ...
    async def init_async(self) -> None:
        ...
        # MIoT cert client
        self._cert = MIoTCert(
            storage=self._storage, uid=self._uid, cloud_server=self.cloud_server
        )
        ...
        if self._ctrl_mode == CtrlMode.AUTO:
            ...
            if self._cloud_server in SUPPORT_CENTRAL_GATEWAY_CTRL:
                for home_id, info in self._entry_data["home_selected"].items():
                    ...
                    service_data = self._mips_service.get_services(
                        group_id=info["group_id"]
                    ).get(info["group_id"], None)
                    if not service_data:
                        _LOGGER.info("central mips service not scanned, %s", home_id)
                        continue
                    _LOGGER.info(
                        "central mips service scanned, %s, %s", home_id, service_data
                    )
                    ...
                    mips.sub_mips_state(
                        key=info["group_id"], handler=self.__on_mips_local_state_changed
                    )
                    mips.connect()
            # Lan ctrl
            ...
            self._miot_lan.unsub_lan_state(key=f"{self._uid}-{self._cloud_server}")
            if self._miot_lan.init_done:
    ...
    async def deinit_async(self) -> None:
        self._network.unsub_network_status(key=f"{self._uid}-{self._cloud_server}")
        ...
        self._refresh_props_retry_count = 0
        # Cloud mips
        self._mips_cloud.unsub_mips_state(key=f"{self._uid}-{self._cloud_server}")
    ...
    def hide_non_standard_entities(self) -> bool:
        return self._entry_data.get("hide_non_standard_entities", False)
    ...
    async def refresh_oauth_info_async(self) -> bool:
        try:
            ...
            user_config: dict = await self._storage.load_user_config_async(
                uid=self._uid, cloud_server=self._cloud_server, keys=["auth_info"]
            )
            if (
                not user_config
            ...
            self.__show_client_error_notify(
                message=self._i18n.translate("miot.client.invalid_oauth_info"),  # type: ignore
    ...
    async def refresh_user_cert_async(self) -> bool:
        try:
            ...
            self.__show_client_error_notify(
                message=self._i18n.translate("miot.client.invalid_cert_info"),  # type: ignore
                notify_key="user_cert",
            )
            _LOGGER.error(
                "refresh user cert error, %s, %s", error, traceback.format_exc()
            )
        return False
    ...
    async def set_prop_async(self, did: str, siid: int, piid: int, value: Any) -> bool:
        ...
        if self._ctrl_mode == CtrlMode.AUTO:
            ...
            if (
                device_gw
                and device_gw.get("online", False)
            ...
            if device_lan and device_lan.get("online", False):
                ...
                _LOGGER.debug(
                    "lan set prop, %s.%d.%d, %s -> %s", did, siid, piid, value, result
                )
                rc = (result or {}).get(
                    "code", MIoTErrorCode.CODE_MIPS_INVALID_RESULT.value
                )
        ...
        if device_cloud and device_cloud.get("online", False):
            ...
            if result and len(result) == 1:
                rc = result[0].get("code", MIoTErrorCode.CODE_MIPS_INVALID_RESULT.value)
                if rc in [0, 1]:
                    return True
                if rc in [-704010000, -704042011]:
                    ...
                    self._main_loop.create_task(
                        await self.__refresh_cloud_device_with_dids_async(dids=[did])
                    )
                raise MIoTClientError(self.__get_exec_error_with_rc(rc=rc))
    ...
    async def set_props_async(
        ...
        if self._ctrl_mode == CtrlMode.AUTO:
            ...
            if device_lan and device_lan.get("online", False):
                ...
                rc = {
                    (r or {}).get("code", MIoTErrorCode.CODE_MIPS_INVALID_RESULT.value)
                    for r in result
                }
                if all(t in [0, 1] for t in rc):
                    return True
    ...
    def request_refresh_prop(self, did: str, siid: int, piid: int) -> None:
        if did not in self._device_list_cache:
            raise MIoTClientError(f"did not exist, {did}")
        key: str = f"{did}|{siid}|{piid}"
        if key in self._refresh_props_list:
            return
        self._refresh_props_list[key] = {"did": did, "siid": siid, "piid": piid}
        if self._refresh_props_timer:
            return
        self._refresh_props_timer = self._main_loop.call_later(
            0.2, lambda: self._main_loop.create_task(self.__refresh_props_handler())
        )
    ...
    async def get_prop_async(self, did: str, siid: int, piid: int) -> Any:
        ...
        try:
            if self._network.network_status:
                result = await self._http.get_prop_async(did=did, siid=siid, piid=piid)
                if result:
                    return result
        except Exception as err:  # pylint: disable=broad-exception-caught
            # Catch all exceptions
            _LOGGER.error(
                "client get prop from cloud error, %s, %s", err, traceback.format_exc()
            )
        if self._ctrl_mode == CtrlMode.AUTO:
            # Central hub gateway
            device_gw = self._device_list_gateway.get(did, None)
            if (
                ...
                if mips is None:
                    ...
                    return await mips.get_prop_async(did=did, siid=siid, piid=piid)
            # Lan
            device_lan = self._device_list_lan.get(did, None)
            if device_lan and device_lan.get("online", False):
    ...
    async def action_async(self, did: str, siid: int, aiid: int, in_list: list) -> list:
        if did not in self._device_list_cache:
            raise MIoTClientError(f"did not exist, {did}")
        ...
        if self._ctrl_mode == CtrlMode.AUTO:
            ...
            if device_lan and device_lan.get("online", False):
                ...
                _LOGGER.debug("lan action, %s, %s, %s -> %s", did, siid, aiid, result)
                rc = (result or {}).get(
                    "code", MIoTErrorCode.CODE_MIPS_INVALID_RESULT.value
                )
                if rc in [0, 1]:
        ...
        if device_cloud and device_cloud.get("online", False):
            ...
            if result:
                rc = result.get("code", MIoTErrorCode.CODE_MIPS_INVALID_RESULT.value)
                if rc in [0, 1]:
                    return result.get("out", [])
                if rc in [-704010000, -704042011]:
                    # Device remove or offline
    ...
    def sub_device_state(
        self,
        did: str,
        handler: Callable[[str, MIoTDeviceState, Any], None],
        handler_ctx: Any = None,
    ) -> bool:
    ...
    def __get_exec_error_with_rc(self, rc: int) -> str:
        ...
        if not err_msg:
            ...
            err_msg += f"code={rc}"
        return (
            f"{self._i18n.translate(key='miot.client.device_exec_error')}, " + err_msg
        )
    ...
    def __request_refresh_user_cert(self, delay_sec: int) -> None:
        ...
        self._refresh_cert_timer = self._main_loop.call_later(
            delay_sec,
            lambda: self._main_loop.create_task(self.refresh_user_cert_async()),
        )
    ...
    def __update_device_msg_sub(self, did: str) -> None:
        ...
        _LOGGER.info("device sub changed, %s, from %s to %s", did, from_old, from_new)
    ...
    async def __on_mips_service_state_change(
        ...
        _LOGGER.info("mips service state changed, %s, %s, %s", group_id, state, data)
        ...
        mips.enable_logger(logger=_LOGGER)
        mips.on_dev_list_changed = self.__on_gw_device_list_changed
        mips.sub_mips_state(key=group_id, handler=self.__on_mips_local_state_changed)
        mips.connect()

    @final
    async def __on_mips_cloud_state_changed(self, key: str, state: bool) -> None:
    ...
    @final
    async def __on_mips_local_state_changed(self, group_id: str, state: bool) -> None:
        _LOGGER.info("local mips state changed, %s, %s", group_id, state)
        mips = self._mips_local.get(group_id, None)
        if not mips:
            _LOGGER.error("local mips state changed, mips not exist, %s", group_id)
            return
        if state:
            # Connected
            ...
            for did, info in self._device_list_gateway.items():
                ...
                if did not in self._device_list_cache:
                    ...
                    continue
                self.__update_device_msg_sub(did=did)
                state_old: Optional[bool] = self._device_list_cache.get(did, {}).get(
                    "online", None
                )
                state_new: Optional[bool] = self.__check_device_state(
    ...
    async def __on_miot_lan_state_change(self, state: bool) -> None:
        _LOGGER.info(
            "miot lan state changed, %s, %s, %s", self._uid, self._cloud_server, state
        )
        if state:
            # Update device
            ...
            for did, info in (await self._miot_lan.get_dev_list_async()).items():
                await self.__on_lan_device_state_changed(did=did, state=info, ctx=None)
    ...
    def __on_cloud_device_state_changed(
        ...
        if sub and sub.handler:
            sub.handler(
                did,
                MIoTDeviceState.ONLINE if state_new else MIoTDeviceState.OFFLINE,
                sub.handler_ctx,
            )
        self.__request_show_devices_changed_notify()
    ...
    async def __on_gw_device_list_changed(
        ...
        _LOGGER.info("gateway devices list changed, %s, %s", mips.group_id, did_list)
        ...
        await self.__update_devices_from_gw_async(
            ...
            filter_dids=[
                did
                for did in did_list
                if self._device_list_gateway.get(did, {}).get("group_id", None)
                == mips.group_id
            ],
        )
        self.__request_show_devices_changed_notify()
    ...
    async def __on_lan_device_state_changed(
        ...
        if lan_state_new == self._device_list_lan[did].get(
            "online", False
        ) and lan_sub_new == self._device_list_lan[did].get("push_available", False):
            return
    ...
    async def __load_cache_device_async(self) -> None:
        ...
        cache_list: Optional[dict[str, dict]] = await self._storage.load_async(
            domain="miot_devices", name=f"{self._uid}_{self._cloud_server}", type_=dict
        )  # type: ignore
        if not cache_list:
            self.__show_client_error_notify(
                message=self._i18n.translate("miot.client.invalid_device_cache"),  # type: ignore
    ...
    async def __update_devices_from_cloud_async(
        self, cloud_list: dict[str, dict], filter_dids: Optional[list[str]] = None
    ) -> None:
        ...
        for did, info in self._device_list_cache.items():
            ...
            cloud_state_old: Optional[bool] = self._device_list_cloud.get(did, {}).get(
                "online", None
            )
    ...
    async def __refresh_cloud_devices_async(self) -> None:
        _LOGGER.debug("refresh cloud devices, %s, %s", self._uid, self._cloud_server)
        self._refresh_cloud_devices_timer = None
        ...
        if not result and "devices" not in result:
            self.__show_client_error_notify(
                message=self._i18n.translate("miot.client.device_cloud_error"),  # type: ignore
    ...
    @final
    async def __refresh_cloud_device_with_dids_async(self, dids: list[str]) -> None:
    ...
    def __request_refresh_cloud_devices(self, immediately=False) -> None:
        _LOGGER.debug(
            "request refresh cloud devices, %s, %s", self._uid, self._cloud_server
        )
        if immediately:
            if self._refresh_cloud_devices_timer:
                self._refresh_cloud_devices_timer.cancel()
        ...
        if self._refresh_cloud_devices_timer:
            return
        self._refresh_cloud_devices_timer = self._main_loop.call_later(
            6, lambda: self._main_loop.create_task(self.__refresh_cloud_devices_async())
        )

    @final
    async def __update_devices_from_gw_async(
        self,
        gw_list: dict[str, dict],
        ...
        for did, info in self._device_list_cache.items():
            ...
            gw_state_old = device_old.get("online", False) if device_old else False
            gw_state_new: bool = False
            device_new = gw_list.pop(did, None)
            if device_new:
                # Update gateway device info
                self._device_list_gateway[did] = {**device_new, "group_id": group_id}
        ...
        for did, info in gw_list.items():
            ...
            if did not in self._device_list_cache:
                continue
            group_id_old: str = self._device_list_cache[did].get("group_id", None)
            self._device_list_cache[did]["group_id"] = group_id
            _LOGGER.info("move device %s from %s to %s", did, group_id_old, group_id)
            self.__update_device_msg_sub(did=did)
            state_old: Optional[bool] = self._device_list_cache[did].get("online", None)
            state_new: Optional[bool] = self.__check_device_state(
                self._device_list_cloud.get(did, {}).get("online", None),
    ...
    @final
    async def __refresh_gw_devices_with_group_id_async(self, group_id: str) -> None:
        """Refresh gateway devices by group_id"""
        _LOGGER.debug("refresh gw devices with group_id, %s", group_id)
        # Remove timer
        ...
        if gw_list is None:
            _LOGGER.error(
                "refresh gw devices with group_id failed, %s, %s", self._uid, group_id
            )
            # Retry until success
            self.__request_refresh_gw_devices_by_group_id(group_id=group_id)
            return
    ...
    def __request_refresh_gw_devices_by_group_id(
        ...
        refresh_timer = self._mips_local_state_changed_timers.get(group_id, None)
        if immediately:
            if refresh_timer:
                self._mips_local_state_changed_timers.pop(group_id, None)
            ...
            self._mips_local_state_changed_timers[group_id] = (
                self._main_loop.call_later(
                    0,
                    lambda: self._main_loop.create_task(
                        self.__refresh_gw_devices_with_group_id_async(group_id=group_id)
                    ),
                )
            )
        if refresh_timer:
            return
        self._mips_local_state_changed_timers[group_id] = self._main_loop.call_later(
            3,
            lambda: self._main_loop.create_task(
                self.__refresh_gw_devices_with_group_id_async(group_id=group_id)
            ),
    ...
    async def __refresh_props_from_cloud(self, patch_len: int = 150) -> bool:
        ...
        try:
            ...
            if request_list:
                _LOGGER.info(
                    "refresh props failed, cloud, %s", list(request_list.keys())
                )
                request_list = None
            return True
        except Exception as err:  # pylint:disable=broad-exception-caught
            _LOGGER.error(
                "refresh props error, cloud, %s, %s", err, traceback.format_exc()
            )
    ...
    async def __refresh_props_from_gw(self) -> bool:
            ...
            request_list[did] = {
                **params,
                "fut": mips_gw.get_prop_async(
                    did=did, siid=params["siid"], piid=params["piid"], timeout_ms=6000
                ),
            }
        results = await asyncio.gather(*[v["fut"] for v in request_list.values()])
        ...
        if succeed_once:
            return True
        _LOGGER.info("refresh props failed, gw, %s", list(request_list.keys()))
        # Add failed request back to the list
        self._refresh_props_list.update(request_list)
        return False
    ...
    async def __refresh_props_from_lan(self) -> bool:
        ...
        if succeed_once:
            return True
        _LOGGER.info("refresh props failed, lan, %s", list(request_list.keys()))
        # Add failed request back to the list
        self._refresh_props_list.update(request_list)
        return False
    ...
    async def __refresh_props_handler(self) -> None:
        ...
        self._refresh_props_timer = self._main_loop.call_later(
            3, lambda: self._main_loop.create_task(self.__refresh_props_handler())
        )
    ...
    def __show_client_error_notify(
        ...
        if message:
            self._persistence_notify(
                ...
                self._i18n.translate(key="miot.client.xiaomi_home_error_title"),  # type: ignore
            ...
            self._persistence_notify(
                f"{DOMAIN}{self._uid}{self._cloud_server}{notify_key}error", None, None
            )
    ...
    def __show_devices_changed_notify(self) -> None:
        ...
        if message != "":
            ...
            self._persistence_notify(
                ...
                self._i18n.translate("miot.client.device_list_changed_title"),  # type: ignore
                self._i18n.translate(
...
async def get_miot_instance_async(
    ...
    # MIoT storage
    storage: Optional[MIoTStorage] = hass.data[DOMAIN].get("miot_storage", None)
    if not storage:
        storage = MIoTStorage(root_path=entry_data["storage_path"], loop=loop)
